...
public class FtpUtils {
    ...
    private static ThreadLocal<FtpUtils> sftpLocal = new ThreadLocal<FtpUtils>();
 
    private FtpUtils(String host, int port, String username, String password) throws Exception {
        JSch jsch = new JSch();
     ...
     * @throws Exception
     */
    public static FtpUtils getSftpUtil(String host, int port, String username, String password) throws Exception {
        //获取本地线程
    	FtpUtils sftpUtil = sftpLocal.get();
        if (null == sftpUtil || !sftpUtil.isConnected()) {
            //将新连接防止本地线程，实现并发处理
            sftpLocal.set(new FtpUtils(host, port, username, password));
        }
     ...
     *
     */
    public void uploadByStream(String directory, String filName,InputStream uploadFile) throws Exception {
        try {      //执行列表展示ls 命令
	        channel.ls(directory);      //执行盘符切换cd 命令
	        channel.cd(directory);
	        channel.put(uploadFile, filName);
	        try {
	            if (uploadFile != null) uploadFile.close();
	        } catch (Exception e) {
	            e.printStackTrace();
	            logger.error(filName + "关闭文件时.....异常!" + e.getMessage());
	        }
        }catch (Exception e) {
            logger.error("【子目录创建中】：",e);
            //创建子目录
